<template lang="pug">
  bread-crumbs(:items="bc")
    mutation-form(
      ...
      :button-text="$t('send')"
      :successMessage="$t('support.successMessage')"
      mutation-name="supportSubmit"
      errors-in-alert
    )
      template(#form)
        validation-provider(:name="$t('support.topic')" rules="required|min:2|max:50" v-slot="{ errors, valid }")
          v-text-field.mb-5(v-model="topic" :label="$t('support.topic')" :error-messages="errors" :success="valid" clearable counter )
        validation-provider(:name="$t('support.text')" rules="required|min:2|max:500" v-slot="{ errors, valid }")
