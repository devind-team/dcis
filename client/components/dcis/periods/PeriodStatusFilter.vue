...
items-data-filter(
  ...
  :title="String($t('dcis.periods.statusFilter.title'))"
  :items="items"
  :get-name="i => String($t(`dcis.periods.statuses.${i.id}`))"
  ...
  setup (props, { emit }) {
    ...
    const messageFunction = (selectedItems: Item[], getName: GetName) => {
      if (selectedItems.length) {
        ...
        return String(t('dcis.periods.statusFilter.noFiltrationMessage'))
      }
    }
