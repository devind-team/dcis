...
items-data-filter(
  ...
  :title="String($t('dcis.periods.organizationFilter.levelFilter.title'))"
  :items="items"
  :get-name="i => String($t(`dcis.periods.organizationFilter.levelFilter.types.${i.id}`))"
  :no-filtration-message="String($t('dcis.periods.organizationFilter.levelFilter.noFiltrationMessage'))"
  ...
  setup (props, { emit }) {
    ...
    const messageFunction = (selectedItems: Item[], getName: GetName) => {
      if (selectedItems.length) {
        ...
        return String(t('dcis.periods.organizationFilter.levelFilter.noFiltrationMessage'))
      }
    }
