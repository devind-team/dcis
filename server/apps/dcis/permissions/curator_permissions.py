...
def can_view_curator_group(user: User):
    """Пропускает пользователей, которые могут просматривать кураторскую группу."""
    if user.has_perm('dcis.view_curatorgroup'):
        return
    raise PermissionDenied('Недостаточно прав для просмотра кураторской группы.')
