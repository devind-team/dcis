...
from apps.dcis.services.curator_services import get_curator_group_new_organizations, get_curator_group_new_users
...
class CuratorGroupQueries(graphene.ObjectType):
    ...
    curator_group_new_organizations = DjangoFilterConnectionField(
        OrganizationType,
        required=True,
        description='Получение организаций не входящих ни в одну кураторскую группу'
    )
    ...
    @staticmethod
    @permission_classes((IsAuthenticated,))
    def resolve_curator_group_new_organizations(
        root: Any,
        info: ResolveInfo,
        *args,
        **kwargs
        ...
        can_view_curator_group(info.context.user)
        return get_curator_group_new_organizations()
