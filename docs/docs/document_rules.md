...
Изменять ограничения периода могут пользователи, обладающие хотя бы одним из следующих свойств:

- пользователь может изменять период;
- `change_period_limitations` - пользователь обладает локальной привилегией,
  позволяющей изменять ограничения конкретного периода.
...
Просматривать сводный отчет и выгрузки периода могут пользователи, обладающие хотя бы одним из следующих свойств:

- пользователь может изменять период;
- `Period in get_user_curator_periods(info.context.user)` - пользователь является куратором для периода;
- `view_period_result` - пользователь обладает локальной привилегией,
  позволяющей просматривать сводный отчет и выгрузки конкретного периода.

Изменять методические рекомендации периода могут пользователи, обладающие хотя бы одним из следующих свойств:

- пользователь может изменять период;
- `change_period_methodical_support` - пользователь обладает локальной привилегией,
  позволяющая изменять методические рекомендации конкретного периода.
