...
- `len(get_user_divisions(info.context.user, Project)) > 0` - пользователь добавлен в дивизион,
   который участвует в проекте сбора;
- `info.context.user in Project.period.periodgroup.users` - пользователь добавлен в группу в сборе.
...
- `delete_document` - локальная привилегия, позволяющая удалять документы из конкретного периода.
